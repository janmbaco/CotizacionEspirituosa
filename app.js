...
app.get('/wsjson', function(req, res) {
    ...
    db.EjecutarSQL("SELECT COUNT(A.Id_Permiso) FROM Permisos A JOIN Perfiles_Permisos PA ON PA.Id_Permiso = A.Id_Permiso WHERE A.Paquete = '" + paquete + "' AND A.funcion = '" + funcion + "' AND PA.Id_Perfil = " + Id_Perfil
        , function(bExito, rowsArray) {
            ...
            if (paquete === "aplicación" && funcion === "VerificarContraseña") {
                ...
                db.EjecutarSQL("SELECT Nombre, Valor FROM Propiedades WHERE Nombre IN ('Contraseña','Sal') ORDER BY Nombre"
                    , function(bExito, rowsArray) {
                        if (!bExito) {
                        ...
                        global.hash(global.replaceAll(req.query.param[0], "'", ""), rowsArray[1][1]
                            , function(err, pass) {
                                if (err) {
                                    EvResultado(false, "No se ha podido verificar la contraseña");
                                    return;
            ...
            if (paquete === "tpvs" && funcion === "EstaRegistradoTPV") {
                ...
                db.EjecutarSQL("SELECT Id_TPV, Nombre, IP FROM TPV WHERE IP = '" + req.ip + "'"
                    , function(bExito, rowsArray) {
                        if (!bExito) {
                            EvResultado(false, rowsArray);
                            return;
            ...
            if (Id_Perfil === -1 && paquete === "tpvs" && funcion === "AñadirTPV") {
                if (req.session.ContraseñaVerificada) {
                    ...
                    tpvs.AñadirTPV(global.replaceAll(req.query.param[0], "'", ""), req.ip
