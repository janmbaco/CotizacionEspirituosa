        ...
        return {
            IniciarSesión: function(ArrayIdGruposBebida, EvResultado) {
                ...
                db.Sesiones.ObtenerNuevoId(function(id) {
                    ...
                    for (var i = 0, j = 1; i < ArrayIdGruposBebida.length; i++, j++) {
                        cmdSQL[j] = "INSERT INTO Grupos_Bebida_Sesiones VALUES(" + id + "," + ArrayIdGruposBebida[i] + ");";
                        cmdSQL[++j] = "INSERT INTO Bebidas_Sesiones \n\
                    ...
                    db.EjecutarSQL(cmdSQL, function(bExito, strMensaje) {
                        if (!global.isUndefined(EvResultado))
            ...
            , ListarCotización: function(EvObtResultados) {
                db.EjecutarSQL("SELECT GB.Id_G_Bebida, GB.Nombre Grupo_Bebida, B.Id_Bebida, B.Nombre Bebida, T.Id_T_Servicio, T.Nombre Tipo_Servicio,  C.Precio, COALESCE(SBS.Cantidad_Servicios, 0), B.Cantidad_Stock / T.Cantidad Cantidad_Restante \n\
                                FROM Cotizacion C \n\
                                ...
                                LEFT JOIN (SELECT COUNT(S.Id_Servicio) Cantidad_Servicios, Id_T_Servicio, Id_Bebida \n\
                                        FROM Servicios S\n\
                                        WHERE S.Id_Sesion = (SELECT Id_Sesion FROM Sesiones WHERE FIN IS NULL)\n\
                                        GROUP BY Id_T_Servicio, Id_Bebida) SBS ON SBS.Id_Bebida = B.Id_Bebida AND T.Id_T_Servicio = SBS.Id_T_Servicio\n\
                                ORDER BY GB.Orden, C.Precio ;\n\
                                ", EvObtResultados);
            }
