    ...
    , servicios = module.exports = (function() {
        return {
            ...
            , ListarServicios: function(EvObtResultados){
                db.EjecutarSQL("SELECT S.Id_Sesion, S.Id_Servicio, S.Id_T_Servicio, TS.Nombre, S.Id_Bebida, B.Nombre, S.Precio, S.Fecha \n\
                                FROM Servicios S\n\
                                JOIN Tipos_Servicio TS ON S.Id_T_Servicio = TS.Id_T_Servicio \n\
                                JOIN Bebida B ON S.Id_Bebida = B.Id_Bebida \n\
                                WHERE Id_Sesion = (SELECT Id_Sesion FROM Sesiones WHERE Fin IS NULL);", EvObtResultados);
            }
            , Servicio: (function() {
                return {
                    Añadir: function(ArrDatosServicio, EvResultado) {
                        db.Servicios.ObtenerNuevoId(
                            function(id) {
                                ...
                                var ArrCmdSql = [];
                                debugger;
                                for (var i = 0, j = 0; i < ArrDatosServicio.length; i++, j++){
                                    ArrCmdSql[j] = "INSERT INTO Servicios SELECT Id_Sesion, " + id + " Id_Servicio," + ArrDatosServicio[i].IdTipoServicio + " Id_T_Servicio, " + ArrDatosServicio[i].IdBebida + " Id_Bebida, "+ i +" Id_Ordinal," + ArrDatosServicio[i].Precio + " Precio, '" + (new global.DateTime()).formats.compound.mySQL + "' Fecha FROM Sesiones WHERE FIN IS NULL;";
                    ...
                    , QuitarDatos: function(IdSesion, IdServicio, ArrDatosServicio, EvResultado) {
                        var ArrCmdSql = [];
                        for (var i = 0; i < ArrDatosServicio.length; i++)
                            ArrCmdSql = ["DELETE FROM Servicios WHERE Id_Sesion = "+IdSesion+" AND Id_Servicio = " + IdServicio + " AND Id_T_Servicio = " + ArrDatosServicio[i].IdTipoServicio + " AND Id_Bebida = " + ArrDatosServicio[i].IdBebida + ";"];
                    ...
                    , Quitar: function(IdSesion, IdServicio, EvResultado){
                        db.EjecutarSQL("DELETE FROM Servicios WHERE Id_Sesion = "+IdSesion +" AND Id_Servicio = "+ IdServicio +";", EvResultado);
                    }
                };
            })()
        };
