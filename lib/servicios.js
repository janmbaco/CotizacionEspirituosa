        ...
        return {
            TiposServicio: (function() {
                return{
                    ...
                    , FijarPrecio: function(Id_TPV, idBebida, idTipoServicio, Precio, EvResultado) {
                        db.EjecutarSQL(["INSERT INTO Acciones(Id_Accion, Tipo_Accion, Descripcion, Id_TPV, Fecha)\n\
                                        SELECT COUNT(Id_Accion) + 1, 3, 'Fijar Precio: Id_Bebida:"+idBebida+", Id_Tipo_Servicio:"+idTipoServicio+"', "+Id_TPV+", '" + (new global.DateTime()).formats.compound.mySQL + "'\n\
                                        FROM Acciones;"
                                        ,"UPDATE Cotizacion SET Precio = " + Precio + ", Fijado = 1 WHERE Id_Bebida = " + idBebida + " AND ID_T_Servicio = " + idTipoServicio + ";"],
                            EvResultado);
                    }
                    , LiberarPrecio: function(Id_TPV, idBebida, idTipoServicio, EvResultado) {
                        db.EjecutarSQL(["INSERT INTO Acciones(Id_Accion, Tipo_Accion, Descripcion, Id_TPV, Fecha)\n\
                                        SELECT COUNT(Id_Accion) + 1, 4, 'Liberar Precio: Id_Bebida:"+idBebida+", Id_Tipo_Servicio:"+idTipoServicio+"', "+Id_TPV+", '" + (new global.DateTime()).formats.compound.mySQL + "'\n\
                                        FROM Acciones;"
                                        ,"UPDATE Cotizacion SET Fijado = 0 WHERE id_Bebida = " + idBebida + " AND Id_T_Servicio = " + idTipoServicio + ";"],
                            EvResultado);
                    }
                    , Cotizar: function(idGrupoBebida, idBebida, idTipoServicio, RelacionInicial, PrecioInicial, Maximo, Minimo, Tramo, EvObtResultado) {
                        ...
                        db.EjecutarSQL("SELECT Precio, IFNULL(Fijado, 0) FROM Cotizacion WHERE Id_Bebida = " + idBebida + " AND Id_T_Servicio = " + idTipoServicio + ";"
                            ...
                            , function(bExito, rowsArray) {
                                ...
                                //console.log("esto es lo que hay: "+rowsArray[0][1]);
                                if (rowsArray[0][1] == 1) {
                                ...
                                db.EjecutarSQL("SELECT Cantidad_Stock, Cantidad_Total, Cantidad_Bebidas \n\
                                    ...
                                    , function(bExito, rowsArray) {
                                        ...
                                        var PrecioActual = global.CalcuarCotización(diff, PrecioInicial, Maximo, Minimo, Tramo);
                    ...
                    Añadir: function(ArrDatosServicio, EvResultado) {
                        db.Servicios.ObtenerNuevoId(
                            function(id) {
                                ...
                                var ArrCmdSql = [];
                                ArrCmdSql[0] = "INSERT INTO Servicios(Id_Sesion, Id_Servicio, Fecha, Id_TPV, Id_Empleado)\n\
                                                 SELECT Id_Sesion, " + id + " Id_Servicio, '" + (new global.DateTime()).formats.compound.mySQL + "' Fecha, '" + ArrDatosServicio[0].Id_TPV + "' Id_TPV, '" + ArrDatosServicio[0].Id_Empleado + "' Id_Empleado \n\
                                                 FROM Sesiones WHERE FIN IS NULL;";
                                for (var i = 0, j = 1; i < ArrDatosServicio.length; i++, j++) {
                                    ArrCmdSql[j] = "INSERT INTO Ordenes(Id_Servicio, Id_Orden, Id_T_Servicio, Id_Bebida, Cantidad, Precio)\n\
                                                        VALUES(" + id + ", " + ArrDatosServicio[i].IdOrden + "," + ArrDatosServicio[i].IdTipoServicio + ", " + ArrDatosServicio[i].IdBebida + ", "+ ArrDatosServicio[i].Cantidad + ", " + ArrDatosServicio[i].Precio + ");";
                                    ArrCmdSql[++j] = "UPDATE Bebidas SET Cantidad_Stock = Cantidad_Stock - (SELECT Cantidad * "+ArrDatosServicio[i].Cantidad +"\n\
                                                                                                            FROM Tipos_Servicio \n\
                                                                                                            WHERE Id_T_Servicio = " + ArrDatosServicio[i].IdTipoServicio + ") \n\
                                                        WHERE Id_Bebida = " + ArrDatosServicio[i].IdBebida + ";";
                                }
                                ArrCmdSql[ArrCmdSql.length] = "INSERT INTO Acciones(Id_Accion, Tipo_Accion, Descripcion, Id_TPV, Fecha)\n\
                                                                SELECT COUNT(Id_Accion) + 1, 2, 'Id_Servicio:" + id + "', " + ArrDatosServicio[0].Id_TPV + ", '" + (new global.DateTime()).formats.compound.mySQL + "'\n\
                                                                FROM Acciones;";
                                ArrCmdSql[ArrCmdSql.length] = "UPDATE TPV SET FECHA = '" + (new global.DateTime()).formats.compound.mySQL + "' WHERE Id_TPV = " + ArrDatosServicio[0].Id_TPV + ";";
