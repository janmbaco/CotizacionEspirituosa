    ...
    , db = module.exports = (function() {
        var archDatabase = path.resolve(".\\basededatos\\CotizacionEspirituosa.db")
            ...
            , arrTablas = [{Tabla: 'Grupos_Bebida', Indice: 'Id_G_Bebida'
                ...
                , {Tabla: 'Grupos_Bebida_Sesiones', Indice: 'CAST(Id_Sesion AS STRING)+CAST(Id_G_Bebida AS STRING)'
                    , CreacionTabla: 'CREATE TABLE IF NOT EXISTS Grupos_Bebida_Sesiones(\n\
                ...
                , {Tabla: 'Servicios', Indice: 'Id_Servicio'
                    , CreacionTabla: 'CREATE TABLE IF NOT EXISTS Servicios ( \n\
                                            ...
                                            ,   Id_Ordinal NUMERIC (3,0) NOT NULL \n\
                                            ,   Precio NUMERIC(5,2) NOT NULL \n\
                                            , 	Fecha DATETIME NOT NULL \n\
                                            ,	PRIMARY KEY (Id_Sesion, Id_Servicio, Id_T_Servicio, Id_Bebida, Id_Ordinal) \n\
                                            );'}
        ...
        , InsertarValoresxDefecto = function() {
            ...
            database.serialize(function() {
                var oResultado = new Resultado();
                database.exec("BEGIN;" + arrValoresxDefecto.join(" ") + "COMMIT;", oResultado.fnResultado);
        ...
        , ExecSQL = function(/* Comando SQL */cmdSQL, /* evento resultante */ EvObtenerResultados, _self) {
            ...
            database.serialize(function() {
                ...
                if (typeof cmdSQL === "string")
                    ...
                    //oResultado.RollBackBle = true;
                    database.exec(/*"BEGIN;" + */cmdSQL.join(" ")/* + "COMMIT;"*/, oResultado.fnResultado);
                }
            });
        ...
        return {
            ...
            , Grupos_Bebida_Sesiones: {
                ObtenerDatosTabla: function(EvObtenerResultados) {
                    ObDatosTabla(arrTablas[global.Grupos_Bebida_Sesiones], EvObtenerResultados);
                }
            }
            , Bebidas_Sesiones: {
                ObtenerDatosTabla: function(EvObtenerResultados) {
            ...
            , Servicios: {
                ...
                , ObtenerNuevoId: function(EvObtenerId) {
                    ExecSQL('SELECT COALESCE(MAX(' + arrTablas[global.Servicios].Indice + '),0)+1 AS Cantidad FROM ' + arrTablas[global.Servicios].Tabla + ' Where Id_Sesion = (SELECT Id_Sesion FROM Sesiones WHERE Fin IS NULL);'
                        , function(bExito, rowsArray) {
                            if (!global.isUndefined(EvObtenerId))
                                if (bExito)
                                    EvObtenerId(rowsArray[0][0]);
                                else
                                    EvObtenerId(0);
                        }
                    );
